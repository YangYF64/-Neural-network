...
void showTest(vector<Sample> testGroup){
    ...
    for (size_t i=0;i<testGroup.size();i++){
        for (size_t j=0;j<testGroup[i].in.size();j++){
            cout<<testGroup[i].in[j]<<"\t";
        }

        cout<<"-- XOR训练结果 :";
        for (size_t j=0;j<testGroup[i].out.size();j++){
...
vector<Sample> getTestData(){
    Util util;
    vector<double> testData=util.getFileData((char*)"test.txt");
    if(testData.size()==0){
        cout<<"载入测试数据失败！"<<endl;
        exit(0);
    }
...
vector<Sample> getTrianData(){
    Util util;
    vector<double> trainData=util.getFileData((char*)"data.txt");
    if(trainData.size()==0){
        cout<<"载入训练数据失败！"<<endl;
        exit(0);
    }
